...
import { FaArrowsAltV, FaTape, FaTransgender } from "react-icons/fa";
...
const IndividualView = () => {
	...
	const nationalityName =
		Object.keys(CountryEnum.enums).find((key) => CountryEnum.enums[key] === specialtyInfo?.nationality) || "N/A";
	...
	}, [individualId, getIndividualStars]);
			...
			<div className="container mx-auto flex flex-col text-white-base pb-8">
				...
				<div className="w-full relative flex justify-around items-center gap-[170px] my-8">
							...
							<div className="w-[124px] h-[124px] rounded-full bg-slate-400 overflow-hidden flex justify-center items-center">
								{individual?.specialtyInfo?.profilePicture && (
									<img
										...
										src={individual?.specialtyInfo?.profilePicture}
										alt=""
									/>
								)}
							</div>
					...
					<div className="flex flex-col items-center gap-3">
						<div className="flex flex-col justify-center items-center">
							<h1 className="font-display text-2xl text-white-base font-bold capitalize">{individual?.fullName}</h1>
							{individual?.role === "MODEL" && (
								<p className="font-body text-white-base/50 text-base font-light capitalize mt-4">
									{getKeyByValue(CountryEnum.enums, individual?.specialtyInfo.nationality)}, {""}{" "}
						...
						<div className="text-base font-light font-body text-center w-[210px] capitalize rounded-lg bg-[#197FE540] px-[5px] py-2">
							{individual?.role?.toLowerCase() === "music_and_sound_engineer"
					...
					<div className="flex flex-col items-center justify-between gap-2">
						<Rating
							...
							maxRating={5}
						/>
						<p className="font-body text-white-base/50 text-base font-light">{t("individualView.projectsCompleted")}</p>
					</div>
				</div>
				<Border />
				{/*About*/}
				<div className="py-5 flex flex-col gap-6 my-8">
					<div className="w-full flex justify-between items-center">
						<h1 className="text-[38px] font-bold font-display">{t("individualView.about")}</h1>
					</div>
				...
				<div className="py-5 flex flex-col gap-6 my-8 relative">
					<div className="w-full flex justify-between items-center">
						<h1 className="text-[38px] font-bold font-display">{t("individualView.portfolio")}</h1>
					</div>
					{individual?.role === "MODEL" ? (
						<>
							<div
								className={`w-full grid ${
									individual?.role === "MODEL" ? "grid-cols-3 gap-x-[7.3rem]" : "grid-cols-4 gap-6"
								} justify-items-center`}
							>
								{individual?.specialtyInfo?.portfolioPictures &&
									individual?.specialtyInfo?.portfolioPictures.length !== 0 &&
									individual?.specialtyInfo?.portfolioPictures.map((portfolio, index) => (
										<div
											key={index}
											className={`${individual?.role === "MODEL" ? "w-[350px] h-full" : "col-span-2 h-[220px] w-full"}`}
										>
											<div
										...
										<div className="col-span-1 h-full">
											<div className="h-full px-[26px] py-[10px] bg-[#27292C] border-thin border-white-base/30 text-sm font-light font-body flex flex-col items-center justify-center">
												<FaTape
													...
													size={30}
												/>
												<div>{specialtyInfo?.bust} cm</div>X<div>{specialtyInfo?.hips} cm</div>X
												<div>{specialtyInfo?.waist} cm</div>
											</div>
										</div>
									</div>
								</div>
								...
								<div className="w-full grid grid-cols-4 gap-4">
									<div className="col-span-2 px-[26px] py-[18px] relative bg-[#27292C] border-thin border-white-base/30 text-md font-light font-body flex justify-center items-center gap-2">
										<MdOutlineCameraAlt
											...
											size={30}
										/>
										<p className="text-center grow capitalize">{individual?.specialtyInfo?.camera}</p>
									</div>
									...
									<div className="col-span-2 px-[26px] py-[18px] relative bg-[#27292C] border-thin border-white-base/30 text-md font-light font-body flex justify-center items-center gap-2">
										...
										<p className="text-center grow capitalize">{individual?.specialtyInfo?.lightning}</p>
									</div>
									<div className="col-span-2 px-[26px] py-[18px] relative bg-[#27292C] border-thin border-white-base/30 text-md font-light font-body flex justify-center items-center gap-2">
										<svg
											xmlns="http://www.w3.org/2000/svg"
										...
										<p className="text-center grow capitalize">{individual?.specialtyInfo?.lense}</p>
									</div>
								</div>
							)}
				...
				{individual?.role === "MODEL" && (
					<>
						<div className="py-5 flex flex-col gap-6 my-8">
							<div className="w-full flex justify-between items-center">
								<h1 className="text-[38px] font-bold font-display">{t("individualView.headshots")}</h1>
							</div>
							<div className="grid grid-cols-3 gap-x-[10rem] justify-items-center">
								{individual?.specialtyInfo?.headShots &&
									individual?.specialtyInfo?.headShots !== 0 &&
									individual?.specialtyInfo?.headShots.map((headShot, index) => (
										<div
											key={index}
				...
				<div className="py-5 flex flex-col gap-6 my-8 h-full">
					<div className="w-full flex justify-between items-center">
						<h1 className="text-[38px] font-bold font-display">
							{individual?.role === "MODEL" ? t("individualView.fullBody") : t("individualView.tiktoksReels")}
						</h1>
					</div>
					{individual?.role === "MODEL" ? (
						<div className="grid grid-cols-4 gap-x-[7.5rem] justify-items-center h-full">
							{individual?.specialtyInfo?.fullBodyShots &&
								individual?.specialtyInfo?.fullBodyShots !== 0 &&
								individual?.specialtyInfo?.fullBodyShots.map((fullBody, index) => (
									<div
										key={index}
						...
						<div className="grid grid-cols-4 gap-x-[7.5rem] justify-items-center h-full">
							{individual?.specialtyInfo?.reels &&
								...
								individual?.specialtyInfo?.reels.map((reel, index) => (
									<div
										key={index}
										className="flex flex-col justify-center items-center gap-6 w-[230px] min-h-[380px]"
									>
				...
				<div className="py-5 flex flex-col gap-6 my-8">
					<div className="w-full flex justify-between items-center">
						<h1 className="text-[38px] font-bold font-display">{t("individualView.experienceAwards")}</h1>
					</div>
					<div className="flex flex-col gap-3">
						{individual?.workExperience.map(({ company, startDate, endDate, title }, idx) => (
							<div key={idx}>
								<span className="text-[20px] font-body font-bold">
									{dateFormat(startDate)} - {dateFormat(endDate)}{" "}
								</span>
								<span className="font-normal text-md">
									: <span className="capitalize">{title}</span> at <span className="capitalize">{company}</span>
								</span>
							</div>
						))}
					</div>
