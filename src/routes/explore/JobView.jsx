...
import { FaEnvelope, FaInstagram, FaPhoneAlt, FaTiktok, FaUser } from "react-icons/fa";
import { IoIosCopy } from "react-icons/io";
import { MdOutlineAlternateEmail } from "react-icons/md";
import { TbWorld } from "react-icons/tb";
import { useNavigate, useParams } from "react-router-dom";
import { BackButton, Footer, JobDurationContainer, Loading } from "../../components";
...
const MobileModal = ({ currentJob, setModal, t }) => {
	return (
		<motion.div
			initial={{ opacity: 0 }}
			animate={{ opacity: 1 }}
			...
			<div className="flex gap-5 flex-col items-center justify-center w-full">
				<div className="flex items-center justify-between w-full rounded-md bg-slate-400/50 p-3">
					<div className="flex items-center gap-5">
						<FaPhoneAlt size={25} />
						<h1 className="text-[23px] text-center">{currentJob?.organization?.phone}</h1>
					</div>
					<IoIosCopy
						onClick={() => {
							navigator.clipboard.writeText(currentJob?.organization?.phone);
							toast.success(t("jobView.copiedSuccessfully"));
						}}
...
const EmailModal = ({ currentJob, setModal, t }) => {
	return (
		<motion.div
			initial={{ opacity: 0 }}
			animate={{ opacity: 1 }}
			...
			<div className="flex gap-5 flex-col items-center justify-center w-full">
				<div className="flex items-center justify-between w-full rounded-md bg-slate-400/50 p-3">
					<div className="flex items-center gap-2">
						...
						<h1 className="text-[20px] text-center">{currentJob?.organization?.email}</h1>
					</div>
					<IoIosCopy
						onClick={() => {
							navigator.clipboard.writeText(currentJob?.organization?.email);
							toast.success(t("jobView.copiedSuccessfully"));
						}}
						className="rounded-full p-1 hover:bg-blue-400 duration-300 cursor-pointer"
						size={30}
...
const FormModal = ({ currentJob, setModal, t }) => {
	...
	const handleApply = async (e) => {
		...
		try {
			if (!coverInp.trim()) {
				toast.error(t("jobView.coverLetterRequired"));
				return;
			}
			if (hasNoFixedSalary && (!proposedCost || parseFloat(proposedCost) <= 0)) {
				toast.error(t("jobView.pleaseSpecifyCost"));
				return;
			}
			await sendMessage({
			...
			toast.success(t("jobView.applicationSentSuccess"));
			setModal(null);
			navigate("/messages");
		} catch (err) {
			if (err?.response?.data?.message === "NOT_ALLOWED") {
				toast.error(t("jobView.alreadyApplied"));
				setModal(null);
				return;
			} else {
				toast.error(err?.response?.data?.message || t("jobView.failedToApply"));
			}
		}
	};
	...
	return (
		...
		>
			<h1 className="text-2xl font-bold font-body mb-[15px]">{t("jobView.applyToJob")}</h1>
			<p className="mb-[20px]">
				{currentJob?.salary ? `$${currentJob?.salary}` : t("jobView.costToBeSpecified")} -{" "}
				{getCountryByCode(currentJob?.location)}, {currentJob?.location} - <JobDurationContainer job={currentJob} />
			</p>

			{hasNoFixedSalary && (
				<div className="flex flex-col gap-1 mb-4">
					<h1>{t("jobView.yourProposedCost")}</h1>
					<input
						type="number"
						min="0"
						step="0.01"
						...
						placeholder={t("jobView.enterProposedCost")}
						required
					/>
				</div>
			)}
		...
		const getCurrentJob = async () => {
			try {
				...
				if (data?.requirements) {
					try {
						const parsed = typeof data.requirements === "string" ? JSON.parse(data.requirements) : data.requirements;
				...
				toast.error(err?.response?.data?.message || t("jobView.failedToGetJob"));
			}
		};

		getCurrentJob();
	}, [jobId, getJob, getJobRequests, t]);
				...
				<div className="relative w-full flex justify-around items-center gap-[170px] my-8">
					...
					<div className="flex flex-col items-center gap-5">
						<div className="flex flex-col justify-center items-center">
							<h1 className="font-display text-2xl font-black capitalize">{currentJob?.organization?.name}</h1>
							<p className="font-body text-white-base/50 text-base font-light capitalize">
								{getCountryByCode(currentJob?.organization?.location)}, {currentJob?.organization?.location}
							</p>
						</div>
						<div className="text-base font-light font-body text-center w-[210px] capitalize rounded-lg bg-[#197FE540] px-[5px] py-2">
							{t("jobView.organization")}
						</div>
					...
					<div className="flex flex-col items-center gap-5">
						...
						{currentJob?.jobStatus === "OPEN" && isIndividual ? (
							...
							>
								{t("jobView.onlyIndividualsCanApply")}
							</button>
						) : (
							<button
								type="button"
					...
					<div className="grid grid-cols-5 gap-6">
						...
						<div className="p-6 col-span-2 bg-[#313131] rounded-lg border-2 border-white/15">
							<div className="flex items-center justify-between mb-2">
								<h2 className="text-xl font-bold">{t("jobView.details")}</h2>
							...
							<p className="text-sm text-gray-400 mt-3">
								{t("jobView.posted")} {timeAgo(currentJob?.createdAt)} –{" "}
								{currentJob?.location === "Remote" ? (
									t("jobView.remote")
								) : (
									<>
										{getCountryByCode(currentJob?.location)}, {currentJob?.location}
									</>
								)}
							</p>
							{reqNumber > 0 ? (
								<p className="mt-3 text-lg">
									<span className="font-bold">{reqNumber}</span> {t("jobView.peopleHaveApplied")}
								</p>
							) : (
								<p className="mt-3 text-base 2xl:text-lg font-bold">{t("jobView.noApplicationsYet")}</p>
							)}
						</div>

						{/* Skills And Tools */}
						<div className="p-6 col-span-3 bg-[#313131] rounded-lg border-2 border-white/15">
							<h2 className="text-xl font-bold mb-2">{t("jobView.skillsAndTools")}</h2>
							<div className="flex flex-wrap gap-2 mt-2">
								{currentJob?.tags.map((skill, index) => (
									<span
										key={index}
							...
							<div className="mt-3">
								<p className="text-sm">
									<span className="font-bold">{t("jobView.experienceNeeded")}</span>{" "}
									{currentJob?.requiredExperience?.minimum} {t("jobView.to")} {currentJob?.requiredExperience?.maximum}{" "}
									{t("jobView.years")}
								</p>
								<p className="text-sm">
									<span className="font-bold">{t("jobView.careerLevel")}</span>{" "}
									{currentJob?.careerLevel === "ENTRY_LEVEL"
										? t("postJob.entryLevel")
										: currentJob?.careerLevel === "MID_LEVEL"
											? t("postJob.midLevel")
											: currentJob?.careerLevel === "SENIOR_LEVEL"
												? t("postJob.senior")
												: currentJob?.careerLevel}
								</p>
								<p className="text-sm">
									<span className="font-bold">{t("jobView.educationLevel")}</span>{" "}
									{currentJob?.educationLevel === "HIGH_SCHOOL"
										? t("postJob.highSchool")
										: currentJob?.educationLevel === "BACHELOR"
											? t("postJob.bachelorsDegree")
											: currentJob?.educationLevel === "OTHER"
												? t("postJob.otherDegree")
												: currentJob?.educationLevel}
								</p>
								<p className="text-sm">
									<span className="font-bold">{t("jobView.tattoosOnBody")}</span> {t("jobView.yes")}
								</p>
								<p className="text-sm">
									<span className="font-bold">{t("jobView.jobSalary")}</span> ${currentJob?.salary}
								</p>
								<p className="text-sm">
									<span className="font-bold">{t("jobView.jobCategory")}</span>{" "}
									<span className="capitalize">
										{currentJob?.jobCategory === "MUSIC_AND_SOUND_ENGINEER"
											? t("individualView.musicAndSoundEngineer")
											: currentJob?.jobCategory.toLowerCase()}
									</span>
								</p>
								<p className="text-sm">
									<span className="font-bold">{t("jobView.jobDuration")}</span>{" "}
						...
						<div className="col-span-3 p-6 bg-[#313131] rounded-lg border-2 border-white/15">
							<h2 className="text-xl font-bold mb-2">{t("jobView.jobDescription")}</h2>
							<p className="text-sm text-gray-300">{currentJob?.description}</p>
						</div>

						{/* Job Requirements */}
						<div className="p-6 col-span-2 bg-[#313131] rounded-lg border-2 border-white/15">
							<h2 className="text-xl font-bold mb-2">{t("jobView.jobRequirements")}</h2>
							<ul className="list-disc pl-5 space-y-2 text-sm text-gray-300">
								{requirements && Array.isArray(requirements) && requirements.length > 0 ? (
									requirements.map((requirement, index) => (
										<li key={requirement.id || index}>{requirement.content || requirement}</li>
									))
								) : (
									<li className="text-gray-500">{t("jobView.noRequirementsSpecified")}</li>
								)}
							</ul>
						</div>
					</div>
