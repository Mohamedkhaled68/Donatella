...
const SendChangeOTP = () => {
	...
	const handleSendOTP = async () => {
		...
		try {
			...
			toast.success(t("sendChangeOTP.otpSentSuccess"));
			if (location?.state === "changeEmail") {
			...
			toast.error(err?.response?.data?.message || t("sendChangeOTP.failedToSendOTP"));
		} finally {
			setLoading(false);
		}
	};
				...
				<div className="flex items-center justify-center mb-5 relative w-full h-[30%]">
					...
					<div className="flex flex-col col-span-2 justify-center items-center gap-3">
						<h1 className="font-display text-[35px] 2xl:text-[46px] font-bold text-center">
							{t("sendChangeOTP.title")}
						</h1>
						<p className="text-[15px] 2xl:text-[20px] font-thin text-slate-300">{t("sendChangeOTP.subtitle")}</p>
					</div>
				</div>
				<div className="flex flex-col justify-around items-center h-[60%] 2xl:h-[60%]">
					<div className="w-[70%] mx-auto">
						<div className="w-full text-center text-[40px]">{userStatus?.verifiedEmail}</div>
					</div>
