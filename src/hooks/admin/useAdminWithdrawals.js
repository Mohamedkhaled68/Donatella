...
export const useAdminWithdrawals = (filters = {}, paginate = { page: 1, limit: 20 }) => {
	...
	return useQuery({
		...
		queryFn: async () => {
			...
			const response = await axios.get(`${baseUrl}/admin/payments/withdraw?${params.toString()}`, {
				headers: {
...
export const useAdminUpdateWithdrawal = () => {
	...
	return useMutation({
		mutationFn: async ({ id, data }) => {
			const response = await axios.patch(`${baseUrl}/admin/payments/withdraw/${id}`, data, {
				headers: {
					"Content-Type": "application/json",
					Authorization: `Bearer ${token}`,
				},
