				...
				<div className="container mx-auto flex flex-col items-center justify-center text-center pt-20">
					...
					<div className="mt-12 grid grid-cols-2 gap-8 px-4">
						{cards.map((card, index) => {
							const category = t("services.categories", { returnObjects: true })[index];
							return (
								<div
									key={index}
									className="bg-white-base min-w-[350px] max-w-[600px] text-black rounded-lg p-6 shadow-lg flex flex-col items-center"
								>
									<h2 className="text-[30px] lg:text-5xl font-display">{category.title}</h2>
									<p className="text-sm lg:text-base font-[DM Sans] text-gray-600 mt-2">{category.slogan}</p>
