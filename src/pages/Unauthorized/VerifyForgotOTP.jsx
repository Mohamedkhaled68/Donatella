		...
		if (newOtp.join("").length === length) {
			...
			try {
				...
				if (location?.state === "changeEmail") {
					navigate(getLanguagePath("/change-email"));
				} else if (location?.state === "changePassword") {
					navigate(getLanguagePath("/change-password"));
				} else {
					navigate(getLanguagePath("/forgot-password"));
				}
			} catch (error) {
				toast.error(error?.response?.data?.message || t("verifyForgotOTP.verifyError"));
			} finally {
				setLoading(false);
			}
		}
	...
	const handleResendOtp = async () => {
		...
		await toast
			.promise(
				reSendOtp(),
				{
					loading: t("verifyForgotOTP.resending"),
					success: t("verifyForgotOTP.resentSuccess"),
					error: (err) => {
						setLoading(false);
			...
			.catch((err) => {
				toast.error(err?.response?.data?.message || t("verifyForgotOTP.resentError"));
			})
			.finally(() => {
				setLoading(false);
			});
		...
		<section className="h-screen relative">
			...
			<div className="absolute bottom-0 left-0 w-full pb-10 flex justify-center items-start">
				<div className="text-sm font-semibold flex items-center gap-[5px] font-body text-[#64748B]">
					{t("verifyForgotOTP.didntReceive")}{" "}
					<div
						onClick={handleResendOtp}
						className="text-blue-primary cursor-pointer"
					>
						{t("verifyForgotOTP.clickHere")}
					</div>
				</div>
			</div>
		</section>
