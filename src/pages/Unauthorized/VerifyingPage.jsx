	...
	const handleChange = async (value, index) => {
		...
		if (newOtp.join("").length === length) {
			...
			try {
				...
				toast.success(t("verifyingPage.verifySuccess"));
			} catch (error) {
				toast.error(error?.response?.data?.message || t("verifyingPage.verifyError"));
			} finally {
				setLoading(false);
			}
		}
	...
	const handleResendOtp = async () => {
		...
		await toast
			.promise(
				reSendOtp(),
				{
					loading: t("verifyingPage.resending"),
					success: t("verifyingPage.resentSuccess"),
					error: (err) => {
						setLoading(false);
			...
			.catch((err) => {
				toast.error(err?.response?.data?.message || t("verifyingPage.resentError"));
			})
			.finally(() => {
				setLoading(false);
			});
		...
		<section className="h-screen relative">
			...
			<div className="container mx-auto pt-20 text-white-base h-full">
				<div className="grid grid-cols-3">
					...
					<div className="flex flex-col justify-center items-center gap-3">
						<h1 className="font-display text-[46px] font-bold text-center">{t("verifyingPage.title")}</h1>
						<p className="font-body text-center text-[#94A3B8] w-[70%] mx-auto text-md font-light">
							{t("verifyingPage.subtitle")}
						</p>
					</div>
				</div>
				<div className="flex flex-col justify-center items-center h-[30%] 2xl:h-[60%]">
					<div
						className="grid grid-cols-4 justify-items-center items-center gap-5 w-full mt-10"
						dir="ltr"
					>
						{otp.map((_, index) => (
							<input
			...
			<div className="absolute bottom-0 left-0 w-full pb-10 flex justify-center items-start">
				<div className="text-sm font-semibold flex items-center gap-[5px] font-body text-[#64748B]">
					{t("verifyingPage.didntReceive")}{" "}
					<div
						onClick={handleResendOtp}
						className="text-blue-primary cursor-pointer"
					>
						{t("verifyingPage.clickHere")}
					</div>
				</div>
			</div>
		</section>
