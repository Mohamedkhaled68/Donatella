...
import { BackButton, FormButton, FormGroup, LogoHeader, WorkExperienceForm } from "../../components";
import TermsModal from "../../components/onBoarding/TermsModal";
import { useI18n } from "../../hooks/useI18n";
import { getExperienceFormGroupData, initialExperienceFormValues } from "../../utils/constants";
...
const ExperienceFormSection = () => {
	...
	const getCategory = (cate) => {
		switch (cate) {
			case "tour guide":
				return t("experienceForm.categories.tourGuide");
			case "beautician":
				return t("experienceForm.categories.beautician");
			case "athlete":
				return t("experienceForm.categories.athlete");
			case "artist":
				return t("experienceForm.categories.artist");
			case "fashionista":
				return t("experienceForm.categories.fashionista");
			case "musician":
				return t("experienceForm.categories.musician");
			default:
				return cate;
		}
	};
	...
	const handleSubmite = (e) => {
		...
		const transformedFormValues = Object.fromEntries(
			Object.entries(formValues).map(([key, value]) => {
				...
				if (!Number.isNaN(value) && value !== "") return [key, Number(value)];
				return [key, value];
			}),
		);
		...
		localStorage.setItem("USER_EXPERIENCE_FORM_DATA", JSON.stringify({ ...transformedFormValues, workExperience }));

		setFormValues(initialExperienceFormValues);
		if (location.state?.category) {
			navigate(`/${currentLanguage}/profile-form?category=${location.state.category.toUpperCase()}`, {
				replace: true,
			});
		} else {
			navigate(`/${currentLanguage}/profile-form?category=${localStorage.getItem("USER_ROLE")}`, {
				replace: true,
			});
		}
	};
	...
	useEffect(() => {
		...
		const hasNonEmptyExperience = workExperience.some(
			(experience) => experience.title && experience.company && experience.startDate && experience.endDate,
		);
	...
	const experienceFormGroupData = getExperienceFormGroupData(t);
				...
				<div className="container mx-auto my-10">
					...
					<form onSubmit={handleSubmite}>
						<div className="flex justify-between items-center flex-wrap gap-4 p-4">
							{experienceFormGroupData.map(({ id, label, placeholder, type, value, name }, index) => (
								<motion.div
									key={id}
								...
								>
									<FormGroup
										label={id === "hasWorkExperience" ? label(category) : label}
										id={id}
							...
							>
								<div className={`flex flex-col gap-2`}>
									<div className="flex justify-between items-center">
										...
										>
											{t("experienceForm.workExperienceLabel")} <span className="capitalize">{category}</span>?
										</label>
									</div>
						...
						<div className="flex justify-between items-center mt-[50px]">
							<div className="flex flex-col gap-7 flex-1">
								...
								<p className="text-sm text-[#64748B] font-extralight">{t("termsAndConditions.termsMessage")}</p>
							</div>
							<div className="flex flex-col items-end flex-1 gap-[17px] pb-5">
								<FormButton
									text={t("experienceForm.next")}
									disabled={disabled}
								/>
							</div>
						</div>
