					...
					<div className="space-y-6">
						...
						<div className="bg-[#27292C] rounded-lg p-6">
							<div className="flex items-center justify-between mb-4">
								...
								<span
									className={`px-4 py-2 rounded-full text-sm font-semibold border ${getStatusColor(withdrawal.status)}`}
								>
									{withdrawal.status}
								</span>
							</div>
							<div className="grid grid-cols-2 gap-4 mt-4">
								<div>
									...
									<p className="text-2xl font-bold">${parseFloat(withdrawal.amount || 0).toFixed(2)}</p>
								</div>
								{withdrawal.processedAmount && (
									<div>
										<p className="text-sm text-gray-400">Processed Amount</p>
										<p className="text-2xl font-bold">${parseFloat(withdrawal.processedAmount || 0).toFixed(2)}</p>
									</div>
								)}
							</div>
						...
						<div className="bg-[#27292C] rounded-lg p-6">
							...
							<div className="grid grid-cols-2 gap-4">
								<div>
									...
									<p className="text-lg font-medium">{user.fullName || user.name || "N/A"}</p>
								</div>
								<div>
								...
								<div>
									...
									<p className="text-lg font-medium">${parseFloat(user.balance || 0).toFixed(2)}</p>
								</div>
								{user.location && (
									<div>
						...
						<div className="bg-[#27292C] rounded-lg p-6">
							...
							<div className="space-y-4">
								<div>
									...
									<p className="text-lg font-medium">{new Date(withdrawal.createdAt).toLocaleString()}</p>
								</div>
								{withdrawal.reservedAt && (
									<div>
										<p className="text-sm text-gray-400">Reserved Date</p>
										<p className="text-lg font-medium">{new Date(withdrawal.reservedAt).toLocaleString()}</p>
									</div>
								)}
								{withdrawal.approvedAt && (
									<div>
										<p className="text-sm text-gray-400">Approved Date</p>
										<p className="text-lg font-medium">{new Date(withdrawal.approvedAt).toLocaleString()}</p>
									</div>
								)}
								{withdrawal.approvedBy && (
									<div>
										<p className="text-sm text-gray-400">Approved By</p>
										<p className="text-lg font-medium font-mono text-sm">{withdrawal.approvedBy}</p>
									</div>
								)}
								{withdrawal.rejectedAt && (
									<div>
										<p className="text-sm text-gray-400">Rejected Date</p>
										<p className="text-lg font-medium">{new Date(withdrawal.rejectedAt).toLocaleString()}</p>
									</div>
								)}
								{withdrawal.rejectedBy && (
									<div>
										<p className="text-sm text-gray-400">Rejected By</p>
										<p className="text-lg font-medium font-mono text-sm">{withdrawal.rejectedBy}</p>
									</div>
								)}
								{withdrawal.processedAt && (
									<div>
										<p className="text-sm text-gray-400">Processed Date</p>
										<p className="text-lg font-medium">{new Date(withdrawal.processedAt).toLocaleString()}</p>
									</div>
								)}
								{withdrawal.processedBy && (
									<div>
										<p className="text-sm text-gray-400">Processed By</p>
										<p className="text-lg font-medium font-mono text-sm">{withdrawal.processedBy}</p>
									</div>
								)}
							</div>
						</div>
						...
						{withdrawal.adminNotes && (
							<div className="bg-[#27292C] rounded-lg p-6">
								...
								<p className="text-gray-300 whitespace-pre-wrap">{withdrawal.adminNotes}</p>
							</div>
						)}
					</div>
				</div>
					...
					<div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70">
						...
						>
							...
							<div className="space-y-4">
								...
								<div>
									<label className="block text-sm font-medium mb-2">Processed Amount</label>
									<input
										type="number"
