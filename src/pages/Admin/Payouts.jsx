...
import { useAdminUpdateWithdrawal, useAdminWithdrawals } from "../../hooks/admin/useAdminWithdrawals";
...
const Payouts = () => {
	...
	const filteredWithdrawals = useMemo(() => {
		...
		return withdrawals.items.filter(
			(w) =>
	...
	const handleUpdate = async () => {
		...
		if (newStatus && newStatus !== currentStatus) {
			...
			if (!validTransitions[currentStatus]?.includes(newStatus)) {
				toast.error(`Invalid status transition: Cannot change from ${currentStatus} to ${newStatus}`);
				return;
			}
		}
			...
			<div className="flex-1 p-4 md:p-6 lg:p-8">
				<div className="container mx-auto max-w-7xl">
					...
					<div className="mb-6 md:mb-8">
						...
						<p className="text-gray-400 text-sm md:text-base">Manage withdrawal requests and view audit logs</p>
					</div>
							...
							>
								<div className="flex items-center justify-between">
									<div>
										...
										<p className="text-2xl md:text-3xl font-bold text-yellow-400">{summaryStats.pending}</p>
									</div>
							...
							>
								<div className="flex items-center justify-between">
									<div>
										...
										<p className="text-2xl md:text-3xl font-bold text-purple-400">{summaryStats.total}</p>
									</div>
									<div className="text-3xl md:text-4xl opacity-50">📊</div>
								</div>
							</motion.div>
						...
						<div className="flex flex-col sm:flex-row gap-2 bg-[#27292C] rounded-lg p-1 border border-gray-700">
							...
							>
								...
								{currentTab === "withdrawals" && withdrawals?.items && (
									<span className="ml-2 px-2 py-0.5 bg-white/20 rounded-full text-xs">{withdrawals.items.length}</span>
								)}
							</button>
							<button
							...
							>
								...
								{currentTab === "audit" && auditLogs?.items && (
									<span className="ml-2 px-2 py-0.5 bg-white/20 rounded-full text-xs">{auditLogs.items.length}</span>
								)}
							</button>
						</div>
					</div>
							...
							<div className="flex flex-col sm:flex-row gap-4">
								...
								>
									...
									{(filters.status || showFilters) && (
										<span className="px-2 py-0.5 bg-blue-500 rounded-full text-xs">{filters.status ? "1" : ""}</span>
									)}
								</button>
							</div>
										...
										{(searchQuery ? filteredWithdrawals : withdrawals?.items || []).map((withdrawal) => (
											...
											>
												<td className="px-6 py-4 whitespace-nowrap">
													<div className="text-sm font-medium">{withdrawal.userId?.slice(0, 8)}...</div>
												</td>
												<td className="px-6 py-4 whitespace-nowrap">
													<div className="text-sm font-semibold">${parseFloat(withdrawal.amount || 0).toFixed(2)}</div>
												...
												<td className="px-6 py-4">
													<div className="text-sm text-gray-300">{withdrawal.accountName}</div>
												</td>
												<td className="px-6 py-4 whitespace-nowrap">
													<span
									...
									<div className="flex gap-2">
										...
										<button
											onClick={() =>
												setPaginate({
													...paginate,
													page: Math.min(withdrawals.totalPages, paginate.page + 1),
												})
											}
											...
											<div className="flex flex-col md:flex-row md:items-center md:justify-between gap-2">
												<div className="flex-1">
													<div className="flex items-center gap-2 mb-2">
														...
														<span className="text-xs text-gray-400">{log.entityType.replace(/_/g, " ")}</span>
													</div>
													{log.description && <p className="text-sm text-gray-300 mb-1">{log.description}</p>}
													{log.previousValue && log.newValue && (
														<div className="text-xs text-gray-400 mt-1">
															<span className="text-gray-500">Changed:</span> {JSON.stringify(log.previousValue)} →{" "}
										...
										<div className="flex gap-2">
											...
											<button
												onClick={() =>
													setPaginate({
														...paginate,
														page: Math.min(auditLogs.totalPages, paginate.page + 1),
													})
												}
					...
					<div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/80 p-4">
						...
						>
							<div className="mb-4">
								...
								<p className="text-gray-300">{confirmationModal.message}</p>
							</div>
							...
							<div className="space-y-4 max-h-[70vh] overflow-y-auto">
								<div>
									...
									>
										...
										{(selectedWithdrawal.status === "REJECTED" || selectedWithdrawal.status === "PROCESSED") && (
											<option value={selectedWithdrawal.status}>{selectedWithdrawal.status}</option>
										)}
									</select>
									...
									{selectedWithdrawal.status === "PENDING" && editForm.status === "REJECTED" && (
										<p className="text-xs text-yellow-400 mt-1">This will release the reserved balance back to user</p>
									)}
								</div>
								...
								{selectedWithdrawal.status === "PENDING" && (
									<div>
										...
										<input
											...
											className="w-full bg-[#1a1a1a] border border-gray-600 rounded-lg px-4 py-2 text-white focus:outline-none focus:border-blue-500"
										/>
										<p className="text-xs text-gray-400 mt-1">Minimum: $50 | Maximum: $50,000</p>
								...
								<div>
									<label className="block text-sm font-medium mb-2 text-gray-300">Processed Amount</label>
									<input
										type="number"
