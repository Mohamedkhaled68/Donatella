...
export const breakLongWords = (text, maxWordLength = 20) => {
	...
	return text
		.split(" ")
		.map((word) =>
			word.length > maxWordLength ? word.match(new RegExp(`.{1,${maxWordLength}}`, "g")).join(" ") : word,
		)
		.join(" ");
};
...
export const isNotEmailOrLink = (value) => {
	...
	const urlRegex = /^(https?:\/\/|HTTPS?:\/\/|Http?:\/\/|HTTP?:\/\/)?([\w-]+(\.[\w-]+)+.*)$/;
...
export function compareTimeUnits(unit1, unit2, type) {
	...
	if (type === "less") return index1 < index2 ? unit1.toLowerCase() : unit2.toLowerCase();
	if (type === "greater") return index1 > index2 ? unit1.toLowerCase() : unit2.toLowerCase();
...
export const isValidDurationRange = (minDuration, minPrefix, maxDuration, maxPrefix) => {
