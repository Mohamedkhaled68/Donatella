...
const JobCard = ({ job }) => {
	const { organization, tags, id, jobDuration, location, title, salary, proposalStatus } = job;
	...
	return (
		<div className="flex items-center bg-[#313131] text-white border-thin border-white-base rounded-lg p-6 gap-5 space-x-4 shadow-lg ">
			...
			<div className="flex-1 grow">
				<div className="flex items-center justify-between w-[50%] pb-3 border-b border-white-base/10">
					<h3 className="font-bold">
						<span className="text-white-base text-md font-bold font-display capitalize">{organization?.name} </span>
				...
				<p className="text-[22px] font-bold text-white-base w-[80%] mt-2">{title}</p>
			...
			<div className="flex flex-col justify-center items-center gap-3">
				<p className="text-[20px] font-bold">{salary ? `${salary}$` : "Negotiable"}</p>
