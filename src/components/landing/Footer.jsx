import { FaFacebook, FaInstagramSquare, FaLinkedin, FaSnapchatGhost, FaTiktok, FaYoutube } from "react-icons/fa";
			...
			<div className="container mx-auto grid grid-cols-1 lg:grid-cols-2">
				<div className="flex flex-col items-center lg:items-start gap-11">
					...
					<p className="hidden lg:block text-[17px] font-extralight">
						<span className="font-bold">&copy;</span> {t("footer.allRightsReserved")}, {new Date().getFullYear()}.
					</p>
				</div>
				<div className="flex flex-col mt-[20px] lg:mt-[0] items-center lg:items-end gap-5 lg:gap-11">
					<div className="flex flex-col lg:flex-row gap-3 lg:gap-0 justify-center items-center">
						<p className="font-body text-[18px] lg:text-large font-extralight">{t("footer.findUsAt")}</p>
					...
					<ul className="lg:list-disc flex gap-[8px] lg:gap-12 font-poppins text-[11px] lg:text-base font-light">
						<li>
							<Link to={getLanguagePath("/about")}>{t("footer.links.aboutUs")}</Link>
						</li>
						<li>
							<Link to={getLanguagePath("/privacy")}>{t("footer.links.privacyPolicy")}</Link>
						</li>
						<li>
							<Link to={getLanguagePath("/services")}>{t("footer.links.services")}</Link>
						</li>
						<li>
							<Link to={getLanguagePath("/contact")}>{t("footer.links.contactUs")}</Link>
						</li>
					</ul>
