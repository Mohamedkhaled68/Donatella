...
const Models = ({ modelsRef, talentsRef }) => {
	...
	const handleTalentBtn = (r) => {
		if (isMobile) {
			...
			toast.error(t("models.alreadyLoggedIn"));
		} else {
	...
	const detectMobile = () => {
		...
		const _hasMobileTouch = mediaQuery.matches;

		// Use navigator.userAgentData if available (modern browsers)
		if (navigator.userAgentData) {
			const _platform = navigator.userAgentData.platform.toLowerCase();
			const _mobile = navigator.userAgentData.mobile;
		...
		if (navigator.maxTouchPoints && navigator.maxTouchPoints > 1 && /macintosh/i.test(userAgent)) {
			return true;
		}
	...
	return (
		<section className="relative py-10 px-[1rem] container mx-auto border-y-4 border-[#fcfcfc0d] font-body">
			...
			>
				<ModelsCard
					cardId="men"
					title={t("models.men.title")}
					description={t("models.men.description")}
					image={manModel}
					button={t("models.men.button")}
					isMobile={isMobile}
				...
				<ModelsCard
					cardId="women"
					title={t("models.women.title")}
					description={t("models.women.description")}
					image={womanModel}
					button={t("models.women.button")}
					isMobile={isMobile}
					setOpenNote={setOpenNote}
				...
				>
					{openNote.talent && (
						...
						>
							...
							>
								{t("models.talent.mobileNote")}
							</p>
						</div>
					)}
					<h1 className="text-4xl lg:text-6xl font-display text-center">{t("models.talent.title")}</h1>
					<p className="text-center text-[14px] lg:text-large font-extralight text-gray-deep md:w-[40%] mx-auto">
						{t("models.talent.description")}
					</p>
					...
					<div className="flex justify-center items-center gap-5">
						<Button
							...
							className={"font-light text-white-base bg-[#1F2224] text-sm lg:text-base"}
						>
							{t("models.talent.lookForTalent")}
						</Button>
						<Button
							onClick={() => handleTalentBtn("Individual")}
							className={"font-light border-thin border-[#1F2224] text-sm lg:text-base"}
						>
							{t("models.talent.signUpAsTalent")}
						</Button>
					</div>
				</div>
			</div>
