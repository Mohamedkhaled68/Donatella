...
const WithdrawalDetail = ({ withdrawal, isOpen, onClose }) => {
	const { t } = useI18n();
	const { data: history, isLoading } = useGetWithdrawalHistory(isOpen ? withdrawal?.id : null);

	const getActionLabel = (action) => {
		switch (action) {
			case "UPDATE_WITHDRAWAL_STATUS":
				return t("payments.statusChanged");
			case "UPDATE_WITHDRAWAL_AMOUNT":
				return t("payments.amountChanged");
			case "UPDATE_PROCESSED_AMOUNT":
				return t("payments.processedAmountUpdated");
			case "ADD_ADMIN_NOTES":
				return t("payments.adminNotesAdded");
			case "PROCESS_WITHDRAWAL":
				return t("payments.withdrawalProcessed");
			case "REJECT_WITHDRAWAL":
				return t("payments.withdrawalRejected");
			default:
				return action;
		}
	};
			...
			<div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70">
				...
				>
					<div className="flex justify-between items-center mb-6">
						<h2 className="text-2xl font-bold">{t("payments.withdrawalDetails")}</h2>
						<button
							onClick={onClose}
					...
					<div className="space-y-4 mb-6">
						<div className="bg-[#1a1a1a] rounded-lg p-4">
							<div className="grid grid-cols-2 gap-4">
								<div>
									<p className="text-sm text-gray-400">{t("payments.amount")}</p>
									<p className="text-xl font-bold">${parseFloat(withdrawal.amount || 0).toFixed(2)}</p>
								</div>
								<div>
									<p className="text-sm text-gray-400">{t("payments.status")}</p>
									<span
								...
								<div>
									<p className="text-sm text-gray-400">{t("payments.iban")}</p>
									<p className="text-sm font-mono">{withdrawal.iban?.replace(/(.{4})/g, "$1 ").trim()}</p>
								</div>
								<div>
									<p className="text-sm text-gray-400">{t("payments.accountName")}</p>
					...
					{/* Transaction History */}
					<div>
						<h3 className="text-xl font-bold mb-4">{t("payments.transactionHistory")}</h3>
						{isLoading ? (
										...
										<div className="flex justify-between items-start">
											<div className="flex-1">
												...
												<p className="text-xs text-gray-400 mt-1">
													{formatChange(entry.action, entry.previousValue, entry.newValue)}
												</p>
							...
							<div className="text-center py-8 text-gray-400">{t("payments.noTransactionHistory")}</div>
						)}
					</div>
