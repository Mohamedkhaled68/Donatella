...
const BeautyProfileForm = ({ imageUrls, loading, setLoading }) => {
	const [formValues, setFormValues] = useState(initialBeautyProfileFormValues);
	...
	return (
		<div className="flex flex-col gap-5">
			<div>
				<h1 className="capitalize text-5xl font-display font-bold">{t("profileForms.beauty.title")}</h1>
				...
				<div className="w-[90%] flex flex-col gap-8">
					...
					>
						<div className="flex justify-between items-center">
							...
							>
								{t("profileForms.beauty.highLevelExperience")}
							</label>
						</div>
						<select
					...
					>
						<div className="flex justify-between items-center">
							<label
								...
								htmlFor={"protofolio"}
							>
								{t("profileForms.beauty.portfolioLinks")}
							</label>
						</div>
						<TextInput
							...
							id={"protofolio"}
							type={"text"}
							placeholder={t("profileForms.beauty.portfolioPlaceholder")}
						/>
					</motion.div>
					...
					>
						<div className="flex justify-between items-center">
							<div className="text-white-base text-lg font-body font-normal">
								{t("profileForms.beauty.preferredBrands")}
							</div>
						</div>
						<div className="flex items-center gap-5 flex-wrap pl-2">
							{makeupBrands.map(({ id }) => {
									...
									>
										<input
											...
											onChange={() => {
												setFormValues({
													...formValues,
													preferredBrands: [...formValues.preferredBrands, id],
												});
											}}
										/>
										<label
					...
					>
						<div className="flex justify-between items-center">
							...
							>
								{t("profileForms.beauty.formalTraining")}
							</label>
						</div>
						<select
							className="rounded-[28px] bg-transparent border-extra-thin border-[#94A3B8] text-[#94A3B8] text-medium font-body px-[18px] py-[14px] w-full"
