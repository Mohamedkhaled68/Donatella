...
const useGetEnabledCategories = (language = "en") => {
	return useQuery({
		queryKey: ["categories", "enabled", language],
		queryFn: async () => {
			const response = await axios.get(`${baseUrl}/categories?lang=${language}`);
...
const SelectCategory = () => {
	...
	const { data: categories, isLoading } = useGetEnabledCategories(currentLanguage);
	...
	const navigateToForm = () => {
		if (role) {
			...
			const displayTitle = selectedCategory ? selectedCategory.name : categoryMapping[role]?.title || role;
			navigate(getLanguagePath(`/experience-form`), { state: { categoryName: role, category: displayTitle } });
		}
	};
				...
				<div className="relative container mx-auto pt-3 2xl:h-[90%] flex flex-col justify-around items-center">
					...
					>
						<div className="flex flex-col justify-center items-center gap-[14px]">
							<h1 className="text-3xl font-display font-bold text-white-base">
								{t("onBoarding.selectYourSpeciality")}
							</h1>
							<p className="text-sm text-white-base/40 w-[70%] mx-auto text-center">
								{t("onBoarding.createProfileDescription")}
							</p>
						</div>
					</motion.div>
					<div className="flex flex-col justify-center items-center pt-3 grow">
						{isLoading ? (
								...
								{categories.map((category) => {
									...
									return (
										<div
											...
											className={`${
												role === category.name ? "shadow-lg shadow-white-base" : ""
										...
										>
											<img
												className={`h-full w-full object-cover filter grayscale group-hover:grayscale-0 group-hover:scale-110 ${
													role === category.name ? "scale-110 grayscale-0" : ""
												} duration-500`}
												src={image}
												alt={title}
												loading="lazy"
											...
											<h1 className="absolute text-center leading-8 top-[50%] left-[50%] translate-x-[-50%] translate-y-[-50%] text-white-base text-extra-large z-20">
												{/* {title === "Artist" || title === "Visual Arting & Design"
													? "Visual Artist"
													: title} */}
												{currentLanguage === "ar" ? category.displayNameAr : category.displayNameEn}
											</h1>
											<div
							...
							<div className="text-center text-white-base/60">No categories available</div>
						)}
					</div>
					<div className="flex flex-col justify-center items-center gap-3 mt-5">
						<button
							...
							disabled={!role}
							className={`${
								!role ? "cursor-not-allowed bg-[#494B4E]" : "bg-blue-primary"
