...
const LoadImage = ({ label, inputId, onImageChange, className }) => {
	...
	const handleMediaChange = async (e) => {
		...
		if (label.toLowerCase().startsWith("tik") && !file.type.startsWith("video/"))
			return toast.error("Please select a valid video file.");
		if (!label.toLowerCase().startsWith("tik") && !file.type.startsWith("image/"))
			return toast.error("Please select a valid image file.");

		if (file && (file.type.startsWith("image/") || (inputId === "reel" && file.type.startsWith("video/")))) {
			...
			if (file.type.startsWith("video/")) {
				...
				if (file.size > 1 * 1024 * 1024) return toast.error("File size exceeds 1MB limit.");
				setLoading(true);
				try {
					...
					<div className="w-full flex justify-between items-center">
						<div>
							...
							<input
								id={inputId}
								type="file"
								accept={inputId === "reel" ? "video/*" : "image/*"}
