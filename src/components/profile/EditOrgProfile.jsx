...
const EditOrgProfile = () => {
	...
	const handleFileChange = async (event) => {
		...
		if (!file) return;
		if (file.size > 1 * 1024 * 1024) return toast.error(t("orgProfile.fileSizeExceeds"));
	...
	const handleSubmit = async (e) => {
		...
		try {
			...
			toast.success(t("orgProfile.profileUpdatedSuccess"));
			setModal(null);
		} catch (err) {
			toast.error(err?.response?.data?.message || t("orgProfile.failedToUpdateProfile"));
		} finally {
			setLoading(false);
		}
	};
					...
					<div className="flex flex-col items-end grow gap-4">
						...
						<button
							...
							className={`px-10 py-2 w-[30%] ${
								disabled || loading ? "bg-gray-500 user-events-none cursor-not-allowed" : "bg-blue-primary"
							} rounded-full text-white-base text-lg text-center duration-300 flex justify-center items-center`}
						>
							{loading ? <Loading /> : t("orgProfile.done")}
						</button>
					</div>
				</div>
				<div className="mt-5">
					<div className="w-full grid grid-cols-2 gap-x-5 gap-y-3  ">
						<div className="flex flex-col gap-2">
							<label htmlFor="orgName">{t("orgProfile.organizationName")}</label>
							<input
								id="orgName"
								type="text"
								onChange={(e) =>
						...
						<div className="flex flex-col gap-2">
							<label htmlFor="location">{t("orgProfile.location")}</label>
							<select
								...
								id="location"
							>
								<option value="">{t("orgProfile.selectCountry")}</option>
						...
						<div className="flex flex-col gap-2">
							<label htmlFor="instagram">{t("orgProfile.instagram")}</label>
							<input
								id="instagram"
								type="text"
								onChange={(e) =>
						...
						<div className="flex flex-col gap-2">
							<label htmlFor="tiktok">{t("orgProfile.tiktok")}</label>
							<input
								id="tiktok"
								type="text"
								onChange={(e) =>
						...
						<div className="flex flex-col gap-2">
							<label htmlFor="website">{t("orgProfile.website")}</label>
							<input
								id="website"
								type="text"
								onChange={(e) =>
						...
						<div className="flex flex-col gap-2">
							<label htmlFor="phone">{t("orgProfile.phone")}</label>
							<input
								id="phone"
								type="text"
								onChange={(e) =>
						...
						<div className="flex flex-col col-span-2 gap-2">
							<label htmlFor="bio">{t("orgProfile.bio")}</label>
							<textarea
								id="bio"
								rows={5}
								onChange={(e) =>
