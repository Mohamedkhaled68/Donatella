...
const ModelSpecialtyInfo = ({ userStatus }) => {
	...
	const formattedDate = birthDate ? new Date(birthDate).toLocaleDateString("en-GB") : "N/A";

	const nationalityName = Object.keys(CountryEnum.enums).find((key) => CountryEnum.enums[key] === nationality) || "N/A";
					...
					<div className="col-span-1 h-full">
						<div className="h-full px-[26px] py-[10px] bg-[#27292C] border-thin border-white-base/30 text-sm font-light font-body flex flex-col items-center justify-center">
							<FaTape
								...
								size={30}
							/>
							<div>{bust} cm</div>X<div>{hips} cm</div>X<div>{waist} cm</div>
						</div>
					</div>
				</div>
			</div>
