...
const PreviousWorkGrid = () => {
	...
	const { t } = useI18n();
	const previousWork = userStatus?.individual?.specialtyInfo?.previousWork || [];

	const items = [...previousWork, ...Array(4 - previousWork.length).fill(null)];
	...
	const handleImageUpload = async (event, index) => {
		...
		try {
			...
			const updatedPreviousWork = [...userStatus.individual.specialtyInfo.previousWork];
			...
			toast.error(err?.response?.data?.message || t("imageUpload.errorUploadingImage"));
			setLoading(null);
		}
	};
	...
	return (
		<div className={`w-full grid grid-cols-4 gap-6 justify-items-center`}>
			{items.map((previousWork, index) => (
				<div
					key={index}
					className={`col-span-2 w-full h-[220px]`}
				>
					{previousWork ? (
						<div
