...
const PortfolioGrid = ({ userStatus }) => {
	...
	const portfolio = userStatus?.individual?.specialtyInfo?.portfolioPictures || [];
	...
	const handleImageUpload = async (event, index) => {
		...
		try {
			...
			const updatedPortfolioPictures = [...userStatus.individual.specialtyInfo.portfolioPictures];
			...
			setError(t("imageUpload.errorUploadingImageTryAgain"));
	...
	return (
		<div
			className={`w-full h-full grid ${
				userStatus?.individual?.role === "MODEL" ? "grid-cols-3 gap-x-[7.3rem]" : "grid-cols-4 gap-6"
			} justify-items-center`}
		>
			{items.map((portfolio, index) => (
				<div
					key={index}
					className={`${userStatus?.individual?.role === "MODEL" ? "w-full h-[450px]" : "col-span-2 h-[220px] w-full"}`}
				>
					{portfolio ? (
						<div
						...
						<label
							className={`${
								userStatus?.individual?.role === "MODEL" ? "h-full" : "col-span-2"
