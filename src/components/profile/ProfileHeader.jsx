...
import { FaInstagram, FaTiktok, FaUser } from "react-icons/fa";
...
const ProfileHeader = ({ data }) => {
	...
	const updateVisibaltyStatus = async (isVisible) => {
		try {
			const { firstName, lastName, fullName, ...rest } = userStatus.individual;
			...
			toast.error(err?.response?.data?.message || t("profile.failedToUpdate"));
		}
	};
	...
	const getUserStars = useCallback(
		async (id) => {
			...
			try {
				...
				// Silently fail - stars are not critical for the UI
	...
	useEffect(() => {
		if (!userStatus?.id || userStatus?.role !== "INDIVIDUAL") return;
		getUserStars(userStatus.id);
	}, [userStatus?.id, getUserStars, userStatus?.role]);
	if (!data) {
		return;
	}
		...
		<div className="w-full flex justify-around items-start gap-[170px] my-8">
			{data.role === "ORGANIZATION" ? (
				<>
					...
					<div className="flex flex-col items-center -mt-[7px]">
						...
						<h1 className="font-display text-2xl font-black capitalize mb-[10px]">{data.organization.name}</h1>
						<p className="mb-[10px] font-body text-white-base/50 text-base font-light capitalize">
							{/* Los Angeles, CA, USA */}
							{getKeyByValue(CountryEnum.enums, data?.organization.location)}, {""} {data?.organization.location}
						</p>
						{/* </div> */}
				...
				<>
					<div className={`flex flex-col justify-center items-center gap-3`}>
						<div className="flex justify-center items-end relative">
							<div className="w-[115px] h-[115px] rounded-full bg-slate-400 overflow-hidden flex justify-center items-center">
								{data?.individual?.specialtyInfo?.profilePicture ? (
									<img
										className="w-full h-full object-cover"
										src={data?.individual?.specialtyInfo?.profilePicture}
										alt=""
									/>
								) : (
									<FaUser
					...
					<div className={`flex flex-col items-center  ${userStatus.individual.role === "MODEL" ? "gap-0" : "gap-3"} `}>
						{/* <div className="flex flex-col justify-center items-center"> */}
						<h1
							className={`font-display text-2xl font-black capitalize ${
								userStatus.individual.role === "MODEL" ? "mb-[20px] -mt-[6px]" : "-mt-[7px]"
							}`}
						>
						...
						{userStatus.individual.role === "MODEL" && (
							<p className="font-body text-white-base/50 text-base font-light capitalize mt-[-10px] mb-[13px]">
								{getKeyByValue(CountryEnum.enums, data?.individual?.specialtyInfo.nationality)}, {""}{" "}
								{data?.individual?.specialtyInfo.nationality}
							</p>
						)}
						{/* </div> */}
						{userStatus.individual.role !== "MODEL" && (
							<p className="font-body text-white-base/50 text-base font-light">0 {t("profile.projectsCompleted")}</p>
						)}

						<div className="text-base font-light font-body text-center w-[210px] capitalize rounded-lg bg-[#197FE540] px-[5px] py-2">
							{data?.individual?.role === "MUSIC_AND_SOUND_ENGINEER"
			...
			{data.role === "INDIVIDUAL" ? (
				<div className="flex flex-col items-center justify-between gap-[27px]">
					...
					>
						<span>{t("profile.profileVisibility")}</span>
						<FaAngleDown size={20} />
