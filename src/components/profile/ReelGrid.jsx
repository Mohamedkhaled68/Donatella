...
const ReelGrid = ({ userStatus }) => {
	...
	const handleReelUpload = async (event, index) => {
		...
		if (!file) return;
		if (file.size > 5 * 1024 * 1024) return toast.error(t("videoUpload.fileSizeExceeds5MB"));
		setLoading(index);
		setError(null);
		...
		try {
			...
			setError(t("videoUpload.errorUploadingReel"));
			setLoading(null);
		}
	};
								...
								<div className="absolute bottom-0 left-0  w-full group-hover:opacity-100 opacity-0 duration-300">
									...
									>
										<div className="w-full overflow-hidden flex justify-center items-center rounded-md border-thin border-white-base/30 h-full p-2 bg-[#3B3B3B]">
											{loading === index ? (
												<div className="text-center text-white">{t("videoUpload.uploading")}</div>
											) : (
												<FaPlus size={25} />
											)}
										</div>
										<input
											onChange={(event) => handleReelUpload(event, index)}
											type="file"
