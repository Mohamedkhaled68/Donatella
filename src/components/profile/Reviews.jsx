...
const Reviews = () => {
	...
	}, [userStatus?.id, getReviews]);

	useEffect(() => {
		if (!userStatus?.id) return;
		fetchReviews();
	}, [userStatus?.id, fetchReviews]);
					...
					<div className="bg-blue-primary p-4 rounded-md w-[60%]">
						<div className="flex justify-between items-start">
							<div className="flex flex-col mb-1">
								<p className="font-bold font-body text-[28px] capitalize">{selectedReview?.organizationName}</p>
								<p className="font-light text-sm font-body text-white-base/50 -mt-[2px] mb-[5px]">
									{formatReviewDate(selectedReview?.createdAt)}
								</p>
								<p className="text-white-base">{selectedReview?.comment}</p>
							</div>
						</div>
						<button
						...
						reviews.map((review) => (
							...
							>
								<div className="review rounded-[20px] bg-[#3B3B3B] w-full h-[200px] border-[2.5px] border-white-base/20 p-[17px] flex flex-col">
									<div className="flex flex-col mb-1">
										<p className="font-bold font-body text-[28px] capitalize">{review?.organizationName}</p>
										<p className="font-light text-sm font-body text-white-base/50 -mt-[2px] mb-[5px]">
									...
									<p className="font-light text-base font-body text-white-base tracking-widest mt-2">
										...
										<span
											...
											onClick={() => handleSeeMore(review)} // Pass the selected review
										>
											{t("profile.seeMore")}
										</span>
									</p>
