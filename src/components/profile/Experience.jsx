...
const Experience = ({ userStatus, setLoading }) => {
	...
	const [experience, setExperience] = useState(userStatus.individual?.workExperience || []);
	const { t } = useI18n();
	...
	const handleInputChange = (index, field, value) => {
		...
		if (field === "startDate" || field === "endDate") {
			...
			if (startDate && endDate && new Date(startDate) >= new Date(endDate)) {
				toast.error(t("experience.startDateBeforeEndDate"));
				return;
			}
		}
		...
		const isExperienceComplete = (ex) => {
			return ex.every(
				({ title, company, startDate, endDate }) =>
					title.trim() !== "" && company.trim() !== "" && startDate.trim() !== "" && endDate.trim() !== "",
			);
		};
	...
	return (
		<div className="py-5 flex flex-col gap-6 my-8">
			<div className="w-full flex justify-between items-center">
				<h1 className="text-[38px] font-bold font-display">{t("experience.title")}</h1>
			...
			{!isEditing ? (
				<div className="flex flex-col gap-3">
					{experience.map(({ company, startDate, endDate, title }, idx) => (
						<div key={idx}>
							<span className="text-[20px] font-body font-bold">
								{dateFormat(startDate)} - {dateFormat(endDate)}{" "}
							</span>
							<span className="font-normal text-md">
								: <span className="capitalize">{title}</span> {t("experience.at")}{" "}
				...
				>
					{experience.map((exp, index) => (
						...
						>
							...
							<input
								...
								value={exp.startDate ? formateDate(exp.startDate) : ""}
							...
							<input
								...
								value={exp.endDate ? formateDate(exp.endDate) : ""}
					...
					<div className="flex items-center gap-2">
						<div
							onClick={() => {
								setIsEditing(false);
								setExperience(userStatus.individual.workExperience);
							}}
