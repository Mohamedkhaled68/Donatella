...
const FullBodyGrid = ({ userStatus }) => {
	...
	const handleImageUpload = async (event, index) => {
		...
		try {
			...
			const updatedFullBodyShots = [...userStatus.individual.specialtyInfo.fullBodyShots];
		...
		<div className="grid grid-cols-4 gap-x-[7.5rem] justify-items-center h-full">
			{items.map((fullBody, index) => (
				...
				>
					...
					{error && (
						<div className="text-red-500 text-sm mt-2 text-center">{error}</div> // Display error message
					)}
				</div>
			))}
		</div>
