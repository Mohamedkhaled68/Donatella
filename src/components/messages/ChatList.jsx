...
const ChatList = ({ currentChat, setCurrentChat, filters = {}, onChatsUpdate }) => {
	...
	}, [filters, onChatsUpdate, getMessages]);
		...
		return (
			<div className="w-[30%] h-full rounded-lg bg-[#313131] overflow-y-auto custom-scrollbar">
				<div className="flex flex-col items-center justify-center h-full text-white-base/60 p-4">
					...
					/>
					<p className="text-center">{t("messages.noMessagesYet")}</p>
					<p className="text-sm text-center mt-2 text-zinc-500">{t("messages.startConversation")}</p>
				</div>
			</div>
		);
	}
							...
							{chat?.friend?.profilePicture ? (
								<img
									...
									onError={(e) => {
										...
										e.target.parentElement.innerHTML = '<FaUser className="text-[#9e9e9ebd]" size={20} />';
									}}
								/>
							) : (
								<FaUser
