...
const ChatWindow = ({ currentChat, setReviewModal, reviewModal }) => {
	...
	}, [fetchMessages]); // Dependency array to run the effect when currentChat changes

	if (!currentChat) {
		return (
			<div className="w-[70%] h-full flex justify-center items-center p-4 rounded-lg bg-[#313131]">
				<div className="flex flex-col items-center justify-center gap-5 text-center">
					<h1 className="drop-shadow text-5xl text-white/90 font-display italic font-bold">{t("messages.welcome")}</h1>
				...
				<div className="border-b-thin border-white-base flex justify-between items-center pb-2 w-full">
					<div className="flex items-center  gap-3">
						...
						<div className="flex flex-col">
							<h2 className="text-white-base font-display text-[16px] font-bold">{currentChat.friend.name}</h2>
							<p className="text-white-base/70 font-body text-sm font-extralight">{t("messages.online")}</p>
						</div>
					</div>
