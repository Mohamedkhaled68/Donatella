...
const Message = ({ message, timestamp, isSender, type, jobRequest, setReviewModal, setFinishedJob, setJob }) => {
	const { t } = useI18n();
	...
	const handleAcceptJob = async () => {
		...
		try {
			...
			toast.success(t("messages.jobAcceptedSuccessfully"));
		} catch (err) {
	...
	const handleRejectJob = async () => {
		...
		try {
			...
			toast.success(t("messages.jobRejectedSuccessfully"));
		} catch (err) {
			toast.error(err?.response?.data?.message);
			console.log(err?.response?.data?.message);
		}
	...
	const handleFinishJob = async () => {
		...
		try {
			...
			toast.success(t("messages.jobFinishedSuccessfully"));
		...
		<>
			{type === "REQUEST" ? (
				...
				>
					...
					>
						<h1 className="text-white-base text-2xl font-semibold">{t("messages.newContract")}</h1>
						<p>
							{jobRequest?.job?.salary
								? `${jobRequest?.job?.salary}/HR`
								: jobRequest?.proposedCost
									? `$${jobRequest?.proposedCost} (${t("messages.proposed")})`
									: t("messages.costToBeNegotiated")}{" "}
							- {getCountryByCode(jobRequest?.job?.location)} - {jobRequest?.job?.jobDuration?.minimum} ~{" "}
							{jobRequest?.job?.jobDuration?.maximum}{" "}
							<span className="capitalize">{jobRequest?.job?.jobDuration?.minimumPrefix.toLowerCase()}</span>
						</p>
						<p>{message}</p>
						{userStatus?.role === "INDIVIDUAL" ? (
							...
							<div className="flex justify-between items-center gap-3 mt-3 w-full">
								{jobRequest?.requestStatus === "REJECTED" ? (
									<div className="text-white-base capitalize text-center border border-white-base font-semibold rounded-3xl py-2 px-16 bg-transparent w-full">
										{jobRequest?.requestStatus.toLowerCase()}
									</div>
								) : jobRequest?.requestStatus === "APPROVED" ? (
									<button
										onClick={handleFinishJob}
										type="button"
										className="bg-background-dark font-semibold rounded-3xl py-2 px-16 text-white-base w-full"
									>
										{t("messages.finishJob")}
									</button>
								) : jobRequest?.requestStatus === "FINISHED" ? (
									<div className="text-white-base capitalize text-center border border-white-base font-semibold rounded-3xl py-2 px-16 bg-transparent w-full">
										{jobRequest?.requestStatus.toLowerCase()}
									</div>
								) : (
